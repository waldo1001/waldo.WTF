...
- [x] **Viva Engage (Yammer pivot)** ✅ (2026-04-23) — full 3-slice implementation: (1) `viva-engage` added as 4th message source with per-community subscription model, `viva_subscriptions` table, incremental cursor, and CLI flags `--viva-list/discover/subscribe/unsubscribe`; (2) replaced Graph-backed `HttpVivaClient` with `HttpYammerClient` against `www.yammer.com/api/v1` so external/guest networks are visible (Graph cannot return them); auth auto-consents Yammer scope via second device-code on first `--viva-discover`; (3) `sync-viva.ts` full `olderThan` cursor walk (up to 50 pages/community) with early-stop at `lastCursorAt`, `--viva-discover` outputs 4-column table including `network_name`, `--viva-subscribe` accepts `<networkId>:<communityId>` colon format for disambiguation. Plans: [plans/done/add-viva-engage-source.md](docs/plans/done/add-viva-engage-source.md), [plans/done/yammer-slice-1-auth-consent-fallback.md](docs/plans/done/yammer-slice-1-auth-consent-fallback.md), [plans/done/yammer-slice-2-yammer-rest-client.md](docs/plans/done/yammer-slice-2-yammer-rest-client.md), [plans/done/yammer-slice-3-sync-cursor-and-cli.md](docs/plans/done/yammer-slice-3-sync-cursor-and-cli.md).
...
- [x] **Timestamped logs** ✅ (2026-04-24) — every `Logger.info` / `Logger.error` line now starts with `[<ISO-UTC>]` so `docker compose logs --tail=N` / `tee`'d output / chat screenshots all show *when* each line was emitted. New `createTimestampedConsoleLogger(clock)` factory reuses the existing `Clock` seam, re-reads `clock.now()` on every call. `src/index.ts` wires it via `systemClock`. Zero content change to existing lines — pure prefix. Plan: [docs/plans/done/timestamped-logs.md](docs/plans/done/timestamped-logs.md).
- [x] **`diagnose_sync_health` MCP tool** ✅ (2026-04-24) — 7th v1 tool. Zero-input, read-only; returns categorized + ranked sync-health findings (`auth`, `rate-limit`, `delta-invalid`, `stale`, `repeated-failure`, `viva-sub-drift`, `never-synced`, `unknown-error`) with `overallStatus` rollup and paste-ready `remediationPrompt` per finding. Bearer tokens + long opaque blobs redacted from interpolated error text. New readers: `MessageStore.getSyncLogRecent(limit)`, `VivaSubscriptionStore.listAll()`. Plan: [docs/plans/done/weekend-8-diagnose-sync-health.md](docs/plans/done/weekend-8-diagnose-sync-health.md). Coverage 98.92% / 90% on the handler, 100% / 93.75% on remediation prompts.
